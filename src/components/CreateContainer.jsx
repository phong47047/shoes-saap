...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    uploadTask.on(
      ...
      (error) => {
        console.log(error);
        setFields(true);
        setMsg('Xảy ra lỗi rồi đại vương ơi: Thử Lại nhé ...');
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          setIsLoading(false);
          setFields(true);
          setMsg('Hình ảnh tải lên thành công');
  ...
  const deleteImage = () => {
    ...
    deleteObject(deleteRef).then(() => {
      ...
      setIsLoading(false);
      setFields(true);
      setMsg('Xóa Hình ảnh  thành công');
      setAlertStatus('success');
      setTimeout(() => {
        setFields(false);
      }, 4000);
  ...
  const saveDetails = () => {
    ...
    try {
      if (!title || !calories || !imageAsset || !price || !category) {
        setFields(true);
        setMsg('nhập thông tin bắc buộc không thể để trống...');
        setAlertStatus('danger');
        setTimeout(() => {
          setFields(false);
          setIsLoading(false);
    ...
    <div className="w-full min-h-screen flex items-center justify-center">
      ...
      >
        ...
        <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
          <GiBananaPeel className="text-xl text-gray-700" />
          <input
            type="text"
            required
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="Đặt tên tiêu đề..."
            className="w-full h-full text-lg bg-transparent font-semibold
            outline-none border-none placeholder:text-gray-400 text text-textColor"
          />
        </div>
                ...
                <>
                  ...
                  >
                    <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                      <GiCloudUpload className="text-gray-500 text-3xl hover:text-gray-700" />
                ...
                <>
                  <div className="relative h-full">
                    ...
                    <button
                      ...
                      onClick={deleteImage}
                    >
                      <GiRecycle className="text-white" />
                    </button>
                  </div>
                </>
              )}
        ...
        <div className="w-full flex flex-col md:flex-row items-center gap-3">
          <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
            <GiBank className="text-gray-700 text-2xl" />
            <input
              type="text"
              required
              value={calories}
          ...
          <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
            <GiBanknote className="text-gray-700 text-2xl" />
            <input
              type="text"
              required
              value={price}
